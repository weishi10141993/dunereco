    ...
    , fCaloAlg ( pset.get<fhicl::ParameterSet>("CaloAlg") )
  {
    ...
    for(size_t cstat=0; cstat<fGeom.Ncryostats(); cstat++){
      ...
      for(size_t tpc=0; tpc<fGeom.NTPC(cryoid); tpc++){
        ...
        auto const& tpcgeo = fGeom.TPC(tpcid);
        float halfwidth   = tpcgeo.ActiveHalfWidth();
        float halfheight  = tpcgeo.ActiveHalfHeight();
        float length      = tpcgeo.ActiveLength();
        auto  tpccenter   = tpcgeo.GetActiveVolumeCenter();
       
        std::cout<<"============== CRYOSTAT "<<cstat<<" / TPC "<<tpc<<"=========================\n";
        printf("TPC active-volume center: (%f,%f,%f)\n",tpccenter.X(),tpccenter.Y(),tpccenter.Z());
        std::cout<<"ActiveHalfWidth : "<<halfwidth<<" cm, halfheight: "<<halfheight<<" cm, length: "<<length<<"\n";
        std::cout<<"Trigger time : "<<clockData.TriggerTime()<<"\n";
        std::cout<<"Tick period  : "<<kTickPeriod<<"\n";

        // Loop planes in TPC 'tpc'
        for(size_t pl=0; pl<wireReadoutGeom.Nplanes(tpcid); pl++){
          auto const planeid = geo::PlaneID(cstat,tpc,pl);
          kNumChannels += wireReadoutGeom.Nwires(planeid);
          float offset = detProp.GetXTicksOffset(pl,tpc,cstat);
          std::cout<<"------------------------------------------------\n";
          std::cout<<"--- PLANE "<<pl<<":  "<<wireReadoutGeom.Nwires(planeid)<<" wires, XTicksOffset: "<<offset<<"\n";
	    ...
	    auto const& tpcgeo = fGeom.TPC(tpcid);
            auto const xyz   = wireReadoutGeom.Plane(geo::PlaneID(cstat,tpc,0)).GetCenter();
            auto const xyz_p = wireReadoutGeom.Plane(planeid).GetCenter();
            //const double dir = +1.0;
	    const double dir = (geo::to_int(tpcgeo.DriftSign()) < 0) ? +1.0 : -1.0;
  ...
  void BlipRecoAlg::RunBlipReco( const art::Event& evt ) {
    ...
    */
    ...
    for(auto const &chan : simchanlist ) {
      if( wireReadoutGeom.View(chan->Channel()) != fCaloPlane ) continue;
    ...
    for(size_t i=0; i<hitlist.size(); i++){
      ...
      hitinfo[i].sumADC       = thisHit->HitSummedADC();
        ...
        for(auto const& planehits : plane_hitsMap.second){
          ...
          for(auto const& hi : planehits.second ){
            ...
            for(size_t dw=1; dw<=5; dw++){
              ...
              if( w1 < 0 || w2 >= (int)wireReadoutGeom.Nwires(geo::PlaneID(0,hc.TPC,hc.Plane)))
                flag=true;
              //otherwise, use channel filter service
              else {
                int ch1 = wireReadoutGeom.PlaneWireToChannel(geo::WireID(0,hc.TPC,hc.Plane,w1));
                int ch2 = wireReadoutGeom.PlaneWireToChannel(geo::WireID(0,hc.TPC,hc.Plane,w2));
        ...
        for(auto& i : hitclusts_planeA ) {
          ...
          for(auto&  hitclusts_planeB : planeMap ) {
            ...
            for(auto const& j : hitclusts_planeB.second ) {
              ...
              geo::Point_t xyz;
              if( !art::ServiceHandle<geo::WireReadout const>()->Get().WireIDsIntersect(wireA,wireB,xyz)) continue;
