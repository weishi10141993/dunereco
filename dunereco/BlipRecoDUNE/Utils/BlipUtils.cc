...
namespace BlipUtils {
  ...
  void CalcTotalDep(float& energy, int& ne_dep, float& ne_anode, SEDVec_t& sedvec){
    ...
    for(auto const &chan : art::ServiceHandle<cheat::BackTrackerService>()->SimChannels()) {
      if( art::ServiceHandle<geo::WireReadout const>()->Get().View(chan->Channel()) != geo::kW ) continue;
  ...
  */
  ...
  blipobj::Blip MakeBlip( std::vector<blipobj::HitClust> const& hcs,
    ...
    for(size_t i=0; i<hcs.size(); i++) {
      ...
      if( hcs[i].NWires > newblip.MaxWireSpan ) {
        ...
        newblip.dYZ         = hcs[i].NWires * art::ServiceHandle<geo::WireReadout const>()->Get().Plane(geo::PlaneID(cryo,tpc,pli)).WirePitch();
      }
      ...
      for(size_t j=i+1; j<hcs.size(); j++){
        ...
        if( hcs[i].IntersectLocations.count(hcs[j].ID) ) {
          ...
          geo::Point_t xyz;
          match3d = art::ServiceHandle<geo::WireReadout const>()->Get().WireIDsIntersect(wireA,wireB,xyz);
          y = xyz.Y();
          z = xyz.Z();
        }
  ...
  int DriftDirX(geo::TPCGeo const& tpcgeom)
  {
    return (geo::to_int(tpcgeom.DriftSign()) < 0) ? +1 : -1;
  }
  ...
  bool DoChannelsIntersect(int ch1, int ch2 ){
    // double y,z;
    auto intersection = art::ServiceHandle<geo::WireReadout const>()->Get().ChannelsIntersect(ch1,ch2);
