...
class BlipAnaTreeDataStruct 
{
  ...
  std::vector<ULong64_t> pds_timestamp;
  int t_offset;
...
{ 
  ...
  */
  ...
  try {
     auto const& waveforms = *evt.getValidHandle<std::vector<raw::OpDetWaveform>>("pdvddaphne:daq");
    ...
    for (auto const& in_wave : waveforms) {
      if(in_wave.ChannelNumber()>=1010 && in_wave.ChannelNumber()<=1081){
	toff=in_wave.TimeStamp();
	break;
      }
    }
	...
	// fData->waveformsADC.back().push_back((float)wfsb3.back());
      ...
      findPeaks(wfsb3, thres, 30, peakIndices, peakValues, widths, integrals, leftFound, rightFound);
      ...
      std::transform(peakIndices.begin(), peakIndices.end(),
		     ...
		     [toff_set](double x) { return x + toff_set; });
      ...
      for(size_t ip = 0; ip < peakValues.size(); ++ip) {
        ...
        fData->TPDS.push_back(peakIndices[ip]);
